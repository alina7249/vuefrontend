    ...
    >
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <!--  导师列表  -->
          <div class="space-y-6">
            <template v-for="instructor in filteredInstructors" :key="">
              <motion.div
                :key="instructor.id"
                :whileHover="{
                  y: -5,
              ...
              >
                ...
                <div class="relative h-48">
                  <img :src="instructor.coverImage" :alt="instructor.name" class="w-full h-full object-cover" />
                ...
                <div class="p-5">
                  ...
                  <div class="mb-4">
                    ...
                    <div class="flex flex-wrap gap-2">
                      <template v-for="(specialty, index) in instructor.specialties" :key="index">
                        <span
                          :key="index"
                  ...
                  <div class="mb-4">
                    ...
                    <div class="flex space-x-2 overflow-x-auto pb-2">
                      <template v-for="(image, index) in instructor.portfolio" :key="index">
                        <div :key="index" class="w-20 h-20 flex-shrink-0">
                          <img
                            :src="image"
                            :alt="`作品 ${index + 1}`"
                  ...
                  <div class="flex space-x-3">
                    ...
                    <button
                      class="px-4 py-2 bg-[#4A5F8B] text-[#F5F7FA] rounded-lg font-medium hover:bg-[#4A5059] transition-colors"
                    >
                      预约指导
                    </button>
                  </div>
            ...
            <template v-if="filteredInstructors.length === 0">
        ...
        <div class="lg:col-span-1 space-y-6">
          ...
          <div class="bg-[#F5F7FA] rounded-xl p-6 shadow-sm border border-[#B8C6D8]">
            ...
            <div class="mb-6">
              ...
              <div class="space-y-2">
                <template v-for="specialty in specialties" :key="">
                  <div :key="specialty" class="flex items-center">
                    <input
                      type="radio"
                      :id="`specialty-${specialty}`"
                      name="specialty"
                      :checked="selectedSpecialty === specialty"
                      @change="() => (state.selectedSpecialty = specialty)"
                      class="h-4 w-4 text-[#4A5F8B] focus:ring-[#4A5F8B] border-[#B8C6D8] bg-[#E6EBF2]"
                    />
                    <label :htmlFor="`specialty-${specialty}`" class="ml-2 text-sm text-[#4A5059]">
                      {{ specialty }}
                    </label>
                  </div>
                </template>
            ...
            <div class="mb-6">
              ...
              <div class="space-y-2">
                <template v-for="type in coachingTypes" :key="">
                  <div :key="type" class="flex items-center">
                    <input
                      ...
                      class="h-4 w-4 text-[#4A5F8B] focus:ring-[#4A5F8B] border-[#B8C6D8] bg-[#E6EBF2]"
                    />
                    <label :htmlFor="`type-${type}`" class="ml-2 text-sm text-[#4A5059]">
                      {{ type }}
                    </label>
                  </div>
                </template>
                ...
                <template v-for="range in priceRanges" :key="">
                  <div :key="range" class="flex items-center">
                    <input
                      ...
                      @change="() => (state.selectedPriceRange = range)"
                      class="h-4 w-4 text-[#4A5F8B] focus:ring-[#4A5F8B] border-[#B8C6D8] bg-[#E6EBF2]"
                    />
                    <label :htmlFor="`price-${range}`" class="ml-2 text-sm text-[#4A5059]">
                      {{ range }}
                    </label>
                  </div>
                </template>
          ...
          <div class="bg-[#F5F7FA] rounded-xl p-6 shadow-sm border border-[#B8C6D8]">
            ...
            <div
              class="space-y-4 relative before:absolute before:content-[''] before:left-4 before:top-[25px] before:bottom-[25px] before:w-0.5 before:bg-[#B8C6D8]"
            >
...
<script setup lang="ts">
  ...
  const mockInstructors = [
    {
      ...
      title: '风光摄影导师',
      avatar:
        'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=landscape%20photographer%20male%20professional%20outdoor&sign=40c357e2b6e150256acfb8f243e292fd',
      coverImage:
        'https://space.coze.cn/api/coze_space/gen_image?image_size=landscape_16_9&prompt=landscape%20photography%20master%20workshop%20mountain%20view&sign=c11aa40a3e0184b247944895ec34c214',
    ...
    },
    {
      ...
      title: '人像摄影导师',
      avatar:
        'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=portrait%20photographer%20female%20professional%20studio&sign=0c00c5f05df90880252f980b049fe44f',
      coverImage:
        'https://space.coze.cn/api/coze_space/gen_image?image_size=landscape_16_9&prompt=portrait%20photography%20workshop%20studio%20lighting&sign=74773791a8eb7b39af923086b45e27e1',
    ...
    },
    {
      ...
      title: '后期修图导师',
      avatar:
        'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=photo%20editor%20male%20professional%20workstation&sign=bcd668474e2f710618deaf25b9036a6c',
      coverImage:
        'https://space.coze.cn/api/coze_space/gen_image?image_size=landscape_16_9&prompt=photo%20editing%20workshop%20post-processing%20tutorial&sign=8126c958d436c2ae320338f0c114a87c',
    ...
    },
    {
      ...
      title: '街头人文导师',
      avatar:
        'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=street%20photographer%20male%20urban%20explorer&sign=04b29bd56dafee8a6fcb66ebf36c2ca6',
      coverImage:
        'https://space.coze.cn/api/coze_space/gen_image?image_size=landscape_16_9&prompt=street%20photography%20workshop%20urban%20documentary&sign=ec47e157ee26a5691af1bb8a160b9bce',
  ...
  const coachingTypes = ['全部', '线上指导', '线下带拍', '作品点评', '技术答疑', '创作方案规划'];
  const priceRanges = ['全部', '300元以下', '300-500元', '500-800元', '800元以上'];
  const state = reactive({
    selectedSpecialty: '全部',
    selectedType: '全部',
    selectedPriceRange: '全部',
    searchTerm: '',
    sortBy: 'recommended',
  });
  // recommended, rating, price-asc, price-desc, experience
  // 过滤导师
  const getFilteredInstructors = () => {
    let instructors = [...mockInstructors]; // 按专业领域过滤

    if (selectedSpecialty !== '全部') {
      instructors = instructors.filter((instructor) => instructor.specialties.includes(selectedSpecialty));
    } // 按指导类型过滤
