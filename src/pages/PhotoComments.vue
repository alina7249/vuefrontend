    ...
    >
          ...
          <div class="w-full md:w-2/3">
            ...
            <div class="flex flex-wrap gap-2 mb-4">
              <template v-for="(tag, index) in mockPhotoPost.tags" :key="index">
                <span :key="index" class="px-2 py-1 bg-[#1E2532] text-[#B8C6D8] text-xs rounded-full">
                  #{{ tag }}
                </span>
              </template>
            </div>
        ...
        <form @submit="handleSubmitComment" class="mb-8">
          <div class="flex space-x-4">
            ...
            <div class="flex-1">
              ...
              <textarea
                :value="commentText"
                @change="(e) => (state.commentText = e.target.value)"
      ...
      <div class="space-y-6">
        <template v-for="comment in comments" :key="">
          <div :key="comment.id" class="bg-[#F5F7FA] rounded-lg p-4 border border-[#B8C6D8]">
            <div class="flex space-x-3">
              <Link :to="`/profile/${comment.userId}`" class="flex-shrink-0">
              ...
              <div class="flex-1">
                ...
                <div class="flex items-center space-x-4">
                  ...
                  <button
                    @click="() => (state.replyingTo = replyingTo === comment.id ? null : comment.id)"
                  ...
                  >
                    ...
                    <div class="flex-1 flex space-x-2">
                      <input
                        type="text"
                        :value="replyText"
                        @change="(e) => (state.replyText = e.target.value)"
                ...
                <template v-if="comment.replies && comment.replies.length > 0">
                  <div class="mt-4 pl-4 border-l-2 border-[#B8C6D8] space-y-4">
                    <template v-for="reply in comment.replies" :key="">
                      <div :key="reply.id" class="flex space-x-3">
                        <Link :to="`/profile/${reply.userId}`" class="flex-shrink-0">
    ...
    comments: [
      {
        ...
        username: '极简摄影师林静',
        avatar:
          'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=minimalist%20photographer%20female%20glasses&sign=bcb6273a0e310c266e722c0131d6e146',
        content:
          '这张作品的几何构图非常出色，线条的运用和光影对比恰到好处。特别喜欢你对空间的处理，留白部分增强了整体的极简美感。请问是使用什么方式调整黑白对比的？',
        ...
        replies: [
          {
            ...
            content:
              '谢谢喜欢！我使用了Lightroom的色调曲线工具进行精细调整，重点强化了明暗交界线的对比，同时保留了阴影和高光的细节，避免过度调整导致的细节丢失。',
      ...
      },
      {
        ...
        username: '建筑摄影师王强',
        avatar:
          'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=architecture%20photographer%20male%20smiling&sign=3c23397344efe1e22c27fde5dd0bd934',
        content:
          '作为一名建筑摄影师，我非常欣赏这种捕捉几何美感的视角。这让我想起了埃姆斯住宅的一些经典摄影作品。请问拍摄时是否有特别等待光线的角度？',
        ...
        replies: [
          {
            ...
            avatar:
              'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=minimalist%20photographer%20male%20serious&sign=fded36172bb86afa4dc326776156459c',
            content:
              '是的，我等待了大约30分钟，直到阳光到达这个特定角度，形成了我想要的光影效果。建筑摄影中，光线的方向和质量对最终效果的影响非常大，值得花时间等待最佳时机。',
      ...
      },
      {
        ...
        username: '摄影学习者小张',
        avatar:
          'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=young%20photographer%20student%20male&sign=c8c88269cfd5ed96c4081bb7a4ed50b8',
        ...
        replies: [
          {
            ...
            avatar:
              'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=minimalist%20photographer%20male%20serious&sign=fded36172bb86afa4dc326776156459c',
            content:
              '我的建议是：1. 放慢脚步，用"框架思维"观察周围环境；2. 寻找重复的图案和线条；3. 尝试从不同角度观察同一物体；4. 使用三分法或对称构图；5. 关注负空间的重要性；6. 练习用单色模式（黑白）观察场景，有助于聚焦于形状和线条。最重要的是多拍多练，培养自己的极简视觉。',
          ...
          },
          {
            ...
            username: '艺术摄影师陈默',
            avatar:
              'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=art%20photographer%20male%20creative&sign=bceaa07bd21b90efedda5c86e7059959',
            content:
              '补充一点，我建议随身携带一个小取景器或者用双手框成矩形来辅助构图，这有助于训练你的眼睛直接识别场景中的几何元素和简洁构图。另外，可以尝试使用定焦镜头，限制自己的视角，从而更专注于构图本身。',
            date: '2023-10-25 09:30',
            likes: 67,
            isLiked: false,
          },
    ...
    const updatedComments = comments.map((comment) => {
      ...
      if (isReply && comment.replies) {
        const updatedReplies = comment.replies.map((reply) => {
          if (reply.id === replyId) {
            return { ...reply, likes: reply.isLiked ? reply.likes - 1 : reply.likes + 1, isLiked: !reply.isLiked };
          }
