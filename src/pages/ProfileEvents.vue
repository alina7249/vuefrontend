    ...
    >
      ...
      <div class="grid grid-cols-2 gap-4 mb-8">
        <motion.button
          ...
          @click="() => (state.activeTab = 'upcoming')"
            ...
            <div class="md:flex">
              ...
              <div class="p-5 md:w-2/3">
                ...
                <div class="flex space-x-2">
                  <button
                    class="flex-1 py-2 text-center bg-[#6B7C93] text-[#F5F7FA] rounded-lg font-medium hover:bg-[#4A5F8B] transition-colors"
                  >
                ...
                <template v-if="event.status === '已结束' && (event as any).feedback">
                  <div class="mt-4 p-3 bg-[#2D3748] rounded-lg">
                    <div class="flex items-center mb-2">
                      <div class="flex items-center mr-2">
                        ...
                        >
                          <i
                            :key="i"
                            :class="`fa-solid fa-star ${i < Math.floor((event as any).feedback.rating) ? 'text-[#4A5F8B]' : 'text-[#B8C6D8]'}`"
                          ></i>
                        </template>
                      </div>
                      <span class="text-sm text-[#F5F7FA]"> 您的评价: {{(event as any).feedback.rating}}/5 </span>
                    </div>
                    <p class="text-sm text-[#F5F7FA] italic">"{{(event as any).feedback.comment}}"</p>
                  </div>
                </template>
              </div>
            </div>
...
<script setup lang="ts">
  ...
  const getFilteredEvents = () => {
    let events = [...mockRegisteredEvents]; // 按状态过滤
