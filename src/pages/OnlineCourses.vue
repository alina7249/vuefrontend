        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              >
                ...
                <div class="p-5">
                  ...
                  <div class="flex flex-wrap gap-2 mb-4">
                    <template v-for="tag in course.tags.slice(0, 4)" :key="tag">
                      <button
                        @click="() => toggleTag(tag)"
                  ...
                  <RouterLink
                    :to="`/course/${course.id}`"
                    class="block w-full py-2 text-center bg-gradient-to-r from-[#4A5F8B] to-[#2D3748] text-[#F5F7FA] rounded-lg font-medium hover:from-[#3A4B6F] hover:to-[#1E2532] transition-colors border border-[#4A5F8B]"
                  >
                    查看课程
                  </RouterLink>
                </div>
            ...
            <template v-else>
              <!-- 文字教程列表 -->
              <div
                v-for="tutorial in filteredTextTutorials"
                :key="tutorial.id"
                v-motion="cardMotion"
                class="bg-[#2D3748] rounded-xl overflow-hidden border border-[#4A5F8B] transition-all shadow-sm"
              >
                ...
                <div class="relative">
                  ...
                  />
                  <div class="absolute top-3 right-3 px-2 py-1 bg-[#4A5F8B] text-[#F5F7FA] rounded-full text-xs font-medium">
                    文字教程
                  </div>
                  <div class="absolute bottom-3 right-3 px-2 py-1 bg-[#1E2532]/80 text-[#B8C6D8] rounded-full text-xs backdrop-blur-sm">
                    {{ tutorial.duration }}
                  </div>
                </div>
                ...
                <div class="p-5">
                  <!-- 分类和级别 -->
                  <div class="flex items-center justify-between mb-2">
                    <span class="text-sm text-[#4A5F8B] font-medium">{{ tutorial.category }}</span>
                    <span class="text-xs px-2 py-1 bg-[#1E2532] text-[#B8C6D8] rounded-full">{{ tutorial.level }}</span>
                  </div>
                  ...
                  <div class="flex items-center mb-4">
                    ...
                    />
                    <div>
                      <p class="text-sm font-medium text-[#F5F7FA]">{{ tutorial.author.name }}</p>
                    </div>
                  </div>
                  ...
                  <!-- 教程标签 -->
                  <div class="flex flex-wrap gap-2 mb-4">
                    <template v-for="tag in tutorial.tags.slice(0, 4)" :key="tag">
                      <button
                        @click="() => toggleTag(tag)"
                        :class="`px-2 py-1 rounded-full text-xs ${
                          selectedTags.includes(tag)
                  ...
                  <RouterLink
                    ...
                    class="block w-full py-2 text-center bg-[#4A5F8B] text-[#F5F7FA] rounded-lg font-medium hover:bg-[#3A4B6F] transition-colors border border-[#4A5F8B]"
                  >
                    阅读教程
                  </RouterLink>
                </div>
            ...
            <!-- 空状态 -->
            <div v-if="(activeTab === 'courses' && filteredCourses.length === 0) || 
                     (activeTab === 'textTutorials' && filteredTextTutorials.length === 0)" 
                 class="col-span-full p-8 bg-[#2D3748] rounded-xl border border-[#4A5F8B] text-center">
              ...
              <h3 class="text-lg font-medium text-[#F5F7FA] mb-2">未找到相关{{ activeTab === 'courses' ? '课程' : '教程' }}</h3>
              <p class="text-[#B8C6D8]">
                请尝试使用不同的关键词或筛选条件
              </p>
            </div>
        ...
        <div class="lg:col-span-1 space-y-6">
          ...
          <div class="bg-[#2D3748] rounded-xl p-6 shadow-sm border border-[#4A5F8B]">
            ...
            <div class="mb-6">
              ...
              <div class="space-y-2">
                <div v-for="type in courseTypes" :key="type" class="flex items-center">
                  <input
                    type="radio"
                    :id="`type-${type}`"
                    name="course-type"
                  ...
                  <label
                    :for="`type-${type}`"
                    class="ml-2 text-sm text-[#B8C6D8]"
                  >
                    {{ type }}
                  </label>
                </div>
            ...
            <div class="mb-6">
              ...
              <div class="space-y-2">
                <div v-for="level in difficultyLevels" :key="level" class="flex items-center">
                  <input
                    type="radio"
                    :id="`level-${level}`"
                    name="difficulty-level"
                    :checked="selectedLevel === level"
                    @change="selectedLevel = level"
                    class="h-4 w-4 text-[#4A5F8B] focus:ring-[#4A5F8B] border-[#4A5F8B] bg-[#1E2532]"
                  />
                  <label
                    :for="`level-${level}`"
            ...
            <!-- 应用筛选按钮 -->
            <button class="w-full py-2 bg-[#4A5F8B] text-[#F5F7FA] rounded-lg font-medium hover:bg-[#3A4B6F] transition-colors border border-[#4A5F8B]">
              应用筛选
            </button>
          </div>
          ...
          <div class="bg-[#2D3748] rounded-xl p-6 shadow-sm border border-[#4A5F8B]">
            ...
            <div class="space-y-4">
              <div v-for="instructor in recommendedInstructors" :key="instructor.id" class="flex items-center justify-between">
          ...
          <div class="bg-gradient-to-r from-[#4A5F8B] to-[#6B7C93] rounded-xl p-6 shadow-sm text-[#F5F7FA]">
            ...
            <button class="w-full py-2 bg-[#F5F7FA] text-[#4A5F8B] font-medium rounded-lg hover:bg-[#B8C6D8] transition-colors">
              立即开通
            </button>
          </div>
        </div>
...
const activeTab = ref<'courses' | 'textTutorials'>('courses')
...
const filteredCourses = computed(() => {
  ...
  if (activeCategory.value !== 'all') {
    courses = courses.filter(course => course.category === activeCategory.value)
  }
  
  // 按类型过滤
  if (selectedType.value !== '全部') {
    courses = courses.filter(course => course.type === selectedType.value)
  }
  ...
  if (selectedTags.value.length > 0) {
    courses = courses.filter(course => 
      selectedTags.value.every(tag => course.tags.includes(tag))
    )
  }
...
// 过滤文字教程
const filteredTextTutorials = computed(() => {
  let tutorials = [...mockTextTutorials]
  
  // 按分类过滤
  if (activeCategory.value !== 'all') {
    tutorials = tutorials.filter(tutorial => tutorial.category === activeCategory.value)
  }
  
  // 按类型过滤
  if (selectedType.value === '文字教程') {
    tutorials = tutorials
  ...
  // 按难度级别过滤
  if (selectedLevel.value !== '全部') {
    tutorials = tutorials.filter(tutorial => tutorial.level === selectedLevel.value)
  }
  ...
  if (selectedTags.value.length > 0) {
    tutorials = tutorials.filter(tutorial => 
      selectedTags.value.every(tag => tutorial.tags.includes(tag))
    )
  }
