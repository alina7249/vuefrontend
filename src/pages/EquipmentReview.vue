      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <div class="bg-[#E6EBF2] rounded-xl shadow-sm border border-[#B8C6D8] overflow-hidden">
            <div class="flex">
              <template v-for="category in reviewCategories" :key="">
                <button
                  :key="category.id"
          ...
          <!--  测评列表  -->
          <div class="space-y-6">
            <template v-for="review in filteredReviews" :key="">
              <motion.div
                :key="review.id"
                :whileHover="{
                  y: -5,
              ...
              >
                ...
                <div class="md:flex">
                  <div class="md:w-1/3">
                    <img :src="review.featuredImage" :alt="review.title" class="w-full h-48 md:h-full object-cover" />
                  </div>
                  ...
                  <div class="p-5 md:w-2/3">
                    ...
                    <div class="flex items-center justify-between mb-3">
                      <span
                        class="px-2.5 py-0.5 rounded-full text-xs font-medium bg-[#E6EBF2] text-[#4A5F8B] border border-[#B8C6D8]/30"
                      >
                        {{ review.type }}
                      </span>
                    ...
                    <div class="flex flex-wrap gap-2 mb-4">
                      <template v-for="(tag, index) in review.tags" :key="index">
                        <button
                          :key="index"
                          @click="() => toggleTag(tag)"
                ...
                <div class="px-5 py-4 bg-[#E6EBF2]/50 border-t border-[#B8C6D8]">
                  ...
                  <div class="flex flex-wrap gap-3">
                    <template v-for=" in Object.entries(review.performance || {}).slice(0, 4)" :key="">
  ...
  width: `${(value as number) * 10}%`
        ...
        <div class="lg:col-span-1 space-y-6">
          ...
          <div class="bg-[#F5F7FA] rounded-xl p-6 shadow-sm border border-[#B8C6D8]">
            ...
            <div class="mb-6">
              ...
              <div class="flex flex-wrap gap-2">
                <template v-for="tag in equipmentTypeTags" :key="">
                  <button
                    :key="tag"
                    @click="() => toggleTag(tag)"
            ...
            <div class="mb-6">
              ...
              <div class="flex flex-wrap gap-2">
                <template v-for="tag in priceRangeTags" :key="">
                  <button
                    :key="tag"
                    @click="() => toggleTag(tag)"
                    :class="`px-3 py-1 rounded-full text-sm ${
            ...
            <div>
              ...
              <div class="flex flex-wrap gap-2">
                <template v-for="tag in usageScenarioTags" :key="">
                  <button
                    :key="tag"
                    @click="() => toggleTag(tag)"
                    :class="`px-3 py-1 rounded-full text-sm ${
          ...
          <div class="bg-[#F5F7FA] rounded-xl p-6 shadow-sm border border-[#B8C6D8]">
            ...
            <div class="space-y-4">
              <template
                v-for="author in [
                  {
                    ...
                    name: '器材专家张明',
                    avatar:
                      'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=photography%20equipment%20expert%20male%20professional&sign=56fa5f34db1fbce04f76c7576c6ad020',
                    reviews: 48,
                    followers: 12543,
                    rating: 9.4,
                  },
                  {
                    ...
                    name: '镜头专家刘芳',
                    avatar:
                      'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=photography%20lens%20expert%20female%20professional&sign=c9dd2373388218683b3e980d22233258',
                    reviews: 36,
                    followers: 9876,
                    rating: 9.5,
                  },
                  {
                    ...
                    name: '婚礼摄影师李华',
                    avatar:
                      'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=wedding%20photographer%20male%20creative&sign=82c2687369cb5518e618423326b5a47c',
                    reviews: 24,
                    followers: 7654,
                    rating: 9.2,
                  },
                ...
                :key=""
              >
                <div :key="author.id" class="flex items-center justify-between">
                  <div class="flex items-center space-x-3">
                    <img
                      :src="author.avatar"
                      :alt="author.name"
                  ...
                  <button
                    class="px-3 py-1 text-xs font-medium text-[#4A5059] border border-[#B8C6D8] rounded-full hover:bg-[#E6EBF2] transition-colors"
                  >
                    关注
                  </button>
                </div>
          ...
          <div class="bg-[#F5F7FA] rounded-xl p-6 shadow-sm border border-[#B8C6D8]">
                ...
                v-for="news in [
                  {
                    ...
                    date: '2023-10-28',
                    image:
                      'https://space.coze.cn/api/coze_space/gen_image?image_size=landscape_4_3&prompt=sony%20fe%2050mm%20f1.4%20gm%20new%20lens%20announcement&sign=065ec6ce07308a1967c2f98357dc845c',
                  },
                  {
                    id: 'n2',
                    ...
                    date: '2023-10-25',
                    image:
                      'https://space.coze.cn/api/coze_space/gen_image?image_size=landscape_4_3&prompt=canon%20eos%20r5%20mark%20ii%20rumor%20specs&sign=972fefe44dd95b251299ec0bd90375c9',
                  },
                  {
                    id: 'n3',
                    ...
                    date: '2023-10-22',
                    image:
                      'https://space.coze.cn/api/coze_space/gen_image?image_size=landscape_4_3&prompt=fujifilm%20x-s20%20firmware%20update%20improvement&sign=e3c3283ac365e9940fb2ef57f8ef0dfd',
                  },
                ]"
                :key=""
              >
            ...
            <Link
              to="#"
              class="mt-4 inline-block text-sm text-[#4A5F8B] hover:underline transition-colors flex items-center"
            >
              <span>查看更多动态</span>
...
<script setup lang="ts">
  ...
  const mockReviews = [
    {
      ...
      rating: 9.4,
      featuredImage:
        'https://space.coze.cn/api/coze_space/gen_image?image_size=landscape_16_9&prompt=sony%20a7rv%20review%20professional%20test%20setup&sign=0f74a0352f70258c33141b5bd423cda9',
      tags: ['索尼', '全画幅', '高像素', '专业', '微单'],
      excerpt:
        '索尼A7R V作为一款高像素全画幅微单相机，带来了诸多技术革新。本文将从画质、对焦、视频、操控等多个维度进行深度测评，帮助您了解这款相机是否值得入手。',
    ...
    },
    {
      ...
      rating: 9.2,
      featuredImage:
        'https://space.coze.cn/api/coze_space/gen_image?image_size=landscape_16_9&prompt=wedding%20photography%20canon%20camera%20real%20usage&sign=eef22ed73ef34e8fa8163b2953ee2828',
      tags: ['佳能', '婚礼摄影', '高速连拍', '微单', '弱光性能'],
      excerpt:
        '作为一名职业婚礼摄影师，我在过去的三个月里使用佳能EOS R6 II拍摄了20多场婚礼。本文将分享我的实际使用体验，包括对焦性能、高感表现、电池续航等关键指标。',
    ...
    },
    {
      ...
      rating: 9.0,
      featuredImage:
        'https://space.coze.cn/api/coze_space/gen_image?image_size=landscape_16_9&prompt=fujifilm%20x-t5%20unboxing%20retro%20design%20modern%20features&sign=0eb54e522fb51913a6c810ff7bc07a4a',
      tags: ['富士', '复古', '街拍', 'APS-C', '高像素'],
      excerpt:
        '富士X-T5以其复古的外观设计和强大的性能吸引了众多摄影爱好者。本文将从开箱体验开始，详细介绍这款相机的外观、功能和实际拍摄表现。',
    ...
    },
    {
      ...
      rating: 9.6,
      featuredImage:
        'https://space.coze.cn/api/coze_space/gen_image?image_size=landscape_16_9&prompt=sony%20fe%2024-70mm%20f2.8%20gm%20ii%20lens%20test%20chart&sign=bf971eeae817dba1e6f998b5430cfbb2',
      tags: ['索尼', '大三元', '标准变焦', '专业', '镜头'],
      excerpt:
        '作为索尼新一代大三元标准变焦镜头，FE 24-70mm F2.8 GM II带来了哪些提升？本文通过实验室测试和实际拍摄，全面解析这款镜头的光学性能。',
  ...
  const getFilteredReviews = () => {
    ...
    if (selectedTags.length > 0) {
      reviews = reviews.filter((review) => review.tags.some((tag) => selectedTags.includes(tag)));
    } // 排序
