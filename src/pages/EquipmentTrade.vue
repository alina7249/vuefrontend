  ...
  <div class="container mx-auto px-4 py-8 bg-[#1E2532] star-texture min-h-screen">
    ...
    >
      ...
      <div class="bg-[#4A5568] rounded-xl p-6 shadow-sm border border-[#4A5568] mb-8">
        <div class="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4 mb-4">
          ...
          <select
            ...
            @change="(e) => (state.selectedPriceRange = e.target.value)"
            class="input appearance-none cursor-pointer"
          >
            <template v-for="range in priceRanges" :key="">
              <option :key="range" :value="range">{{ range }}</option>
            </template>
          </select>

          <template v-if="tradeType === 'used'">
            <select
              :value="selectedCondition"
              @change="(e) => (state.selectedCondition = e.target.value)"
              class="input appearance-none cursor-pointer"
            >
              <template v-for="condition in conditions" :key="">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <template v-for="item in currentEquipment" :key="">
          <motion.div
            :key="item.id"
            :whileHover="{
              y: -5,
          ...
          >
            ...
            <div class="relative">
              ...
              <template v-if="tradeType === 'used'">
                <div
                  class="absolute top-3 left-3 px-2 py-1 bg-[#63B3ED] text-[#0F1C2D] rounded-full text-xs font-medium"
                >
                  {{ item.condition }}
                </div>
              </template>

              <template v-if="item.seller.isOfficial">
                <div
                  class="absolute top-3 left-3 px-2 py-1 bg-[#63B3ED] text-[#0F1C2D] rounded-full text-xs font-medium"
                >
                  官方授权
            ...
            <div class="p-5">
              ...
              <div class="flex items-center mb-4">
                ...
                <template v-if="item.originalPrice !== item.price">
                  <p class="text-sm text-[#718096] line-through ml-2">
              ...
              <template v-if="tradeType === 'used'">
                <div class="space-y-1 mb-4 text-sm">
                  <div class="flex justify-between">
                    ...
                    <span class="text-[#E2E8F0]">{{ item.usageTime }}</span>
                  </div>

                  <template v-if="item.shutterCount">
                    <div class="flex justify-between">
                      <span class="text-[#718096]">快门次数:</span>
                      <span class="text-[#E2E8F0]">{{ item.shutterCount }}</span>
                    </div>
                  </template>

                  <div class="flex justify-between">
                    <span class="text-[#718096]">维修记录:</span>
                    <span class="text-[#E2E8F0]">{{ item.repairHistory }}</span>
                  </div>
                </div>
              </template>
              ...
              <template v-if="tradeType === 'new'">
                <div class="mb-4 text-sm">
                  <div class="flex justify-between">
                    <span class="text-[#718096]">保修:</span>
                    <span class="text-[#E2E8F0]">{{ item.warranty }}</span>
                  </div>
                </div>
              </template>
              ...
              <div class="flex items-center space-x-3">
                ...
                <button
                  class="px-4 py-2 bg-[#63B3ED] text-[#0F1C2D] rounded-lg font-medium hover:bg-[#4299E1] transition-colors"
                >
        ...
        <template v-if="currentEquipment.length === 0">
          <div class="col-span-full p-8 bg-[#4A5568] rounded-xl border border-[#4A5568] text-center">
            <div
              class="w-16 h-16 bg-[#1E2A3A] rounded-full flex items-center justify-center text-[#63B3ED] mx-auto mb-4"
            >
      ...
      <div class="mt-12 bg-[#4A5568] rounded-xl p-6 shadow-sm border border-[#4A5568]">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div class="flex items-start">
            <div
              class="w-10 h-10 rounded-full bg-[#1E2A3A] flex items-center justify-center text-[#63B3ED] mr-3 flex-shrink-0"
            >
...
<script setup lang="ts">
  ...
  const mockUsedEquipment = [
    {
      ...
      originalPrice: '12999',
      image:
        'https://space.coze.cn/api/coze_space/gen_image?image_size=landscape_4_3&prompt=sony%20a7%20iii%20used%20camera%20excellent%20condition&sign=d370c0cc7eb060b61b17c41f64c37106',
      condition: '95新',
      usageTime: '约1年',
      shutterCount: '8500次',
      repairHistory: '无维修记录',
      ...
      seller: {
        ...
        name: '摄影爱好者小王',
        avatar:
          'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=photographer%20male%20smiling%20casual&sign=db92da1c3005295607f7766d7f9263bb',
        location: '上海',
        rating: 4.9,
        completedTransactions: 128,
      },
      description:
        '2022年10月购买，使用非常小心，成色极佳。快门次数仅8500次，无任何磕碰和维修记录。因升级设备故出售，附带全部原装配件。',
      images: [
    ...
    },
    {
      ...
      originalPrice: '13999',
      image:
        'https://space.coze.cn/api/coze_space/gen_image?image_size=landscape_4_3&prompt=canon%20ef%2024-70mm%20f2.8l%20ii%20used%20lens%20good%20condition&sign=574d7e4a7c63e973315252e97f5437a0',
      condition: '9成新',
      usageTime: '约2年',
      shutterCount: '',
      repairHistory: '无维修记录',
      ...
      seller: {
        ...
        name: '专业摄影师老李',
        avatar:
          'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=professional%20photographer%20male%20experienced&sign=fe817dce4d08957c62787348c72eb1b7',
        location: '北京',
        rating: 4.8,
        completedTransactions: 256,
      },
      description:
        '经典佳能大三元标准变焦镜头，2021年购买，使用状况良好。镜片无霉无划痕，对焦快速准确。因更换RF卡口系统故出售。',
      images: [
    ...
    },
    {
      ...
      originalPrice: '7999',
      image:
        'https://space.coze.cn/api/coze_space/gen_image?image_size=landscape_4_3&prompt=fujifilm%20x-t30%20ii%20used%20camera%20kit%20mint%20condition&sign=dd66874018934a8b4acab4d3277af243',
      condition: '99新',
      usageTime: '约3个月',
      shutterCount: '2300次',
      repairHistory: '无维修记录',
      ...
      seller: {
        ...
        name: '新手摄影小张',
        avatar:
          'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=young%20photographer%20male%20student&sign=a076fa14f7977e902fe333f899d2603c',
        location: '广州',
        rating: 4.7,
        completedTransactions: 32,
      },
      description:
        '2023年7月购买，几乎全新，仅使用过几次。因工作繁忙无暇使用故出售。相机和镜头均无任何瑕疵，配件齐全。',
      images: [
    ...
    },
    {
      ...
      originalPrice: '6999',
      image:
        'https://space.coze.cn/api/coze_space/gen_image?image_size=landscape_4_3&prompt=gitzo%20mountaineer%20gt2545t%20used%20tripod%20good%20condition&sign=54fd30452764d988a54fac89f0d1b760',
      condition: '9成新',
      usageTime: '约1.5年',
      shutterCount: '',
      repairHistory: '无维修记录',
      ...
      description:
        '2022年3月购买，碳纤维材质，轻巧耐用，承重能力强。使用状况良好，无任何损坏。因升级更大号三脚架故出售。',
      images: [
  ...
  const mockNewEquipment = [
    {
      ...
      originalPrice: '25999',
      image:
        'https://space.coze.cn/api/coze_space/gen_image?image_size=landscape_4_3&prompt=canon%20eos%20r5%20new%20camera%20professional&sign=540cba86fda3af67e78ac69d90b730e1',
      seller: {
        id: 'b1',
        name: '佳能官方授权店',
        avatar:
          'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=canon%20official%20store%20logo&sign=50cd433cb1c90a4b4dca5af8ff32317b',
        location: '上海',
        rating: 4.9,
      ...
      description:
        '佳能EOS R5是一款专业级全画幅微单相机，具备4500万像素，支持8K视频录制和高速连拍。本商品为全新正品，享受官方保修服务。',
      images: [
    ...
    },
    {
      ...
      originalPrice: '23999',
      image:
        'https://space.coze.cn/api/coze_space/gen_image?image_size=landscape_4_3&prompt=nikon%20z%2070-200mm%20f2.8%20vr%20s%20new%20lens%20professional&sign=38233f6c58be56005f9f35a8b0432feb',
      seller: {
        id: 'b2',
        name: '尼康官方授权店',
        avatar:
          'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=nikon%20official%20store%20logo&sign=1cb76f80ef7e58ff4fd8842daa09e778',
        location: '北京',
        rating: 4.8,
      ...
      description:
        '尼康Z卡口70-200mm F2.8 VR S镜头，采用纳米结晶涂层和ED镜片，提供出色的光学性能和防抖效果。全新正品，支持官方保修。',
      images: [
  ...
  const state = reactive({
    ...
    selectedCondition: '全部',
    searchTerm: '',
    sortBy: 'recommended',
  });
  // recommended, price-asc, price-desc, newest
  // 过滤二手器材
  const getFilteredUsedEquipment = () => {
    let equipment = [...mockUsedEquipment]; // 按类型过滤
  ...
  const getFilteredNewEquipment = () => {
    let equipment = [...mockNewEquipment]; // 按类型过滤
