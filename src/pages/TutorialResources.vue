    ...
    >
      ...
      <div class="bg-[#2D3748] rounded-xl shadow-sm border border-[#4A5F8B] mb-8">
        <div class="flex">
          <button
            ...
            :class="['flex-1 py-4 font-medium transition-colors', activeTab === 'online' ? 'bg-[#4A5F8B] text-[#F5F7FA]' : 'bg-[#2D3748] text-[#B8C6D8] hover:text-[#F5F7FA]']"
          >
            在线工具
          </button>
          <button
            @click="activeTab = 'presets'"
            :class="['flex-1 py-4 font-medium transition-colors', activeTab === 'presets' ? 'bg-[#4A5F8B] text-[#F5F7FA]' : 'bg-[#2D3748] text-[#B8C6D8] hover:text-[#F5F7FA]']"
          >
            个人预设
          </button>
        </div>
      ...
      <div class="bg-[#2D3748] rounded-xl p-6 shadow-sm border border-[#4A5F8B] mb-8">
        ...
        <div class="mt-4">
          ...
          <div class="flex flex-wrap gap-2">
            <button
              v-for="category in tagsCategories"
              :key="category"
              @click="toggleTag(category)"
              :class="['px-3 py-1 rounded-full text-sm transition-colors', selectedTags.includes(category) ? 'bg-[#4A5F8B] text-[#F5F7FA]' : 'bg-[#2D3748] text-[#B8C6D8] border border-[#4A5F8B] hover:bg-[#4A5F8B] hover:text-[#F5F7FA]']"
            >
              #{{ category }}
            </button>
          </div>
      ...
      <template v-if="activeTab === 'online'">
        ...
        <div class="mb-10">
            ...
            >
              ...
              <div class="relative">
                ...
                <div class="absolute top-3 right-3">
                  <span class="px-2 py-1 bg-[#4A5F8B]/80 text-[#F5F7FA] text-xs rounded">{{ tool.category }}</span>
                </div>
              </div>
              ...
              <div class="p-4">
                ...
                <p class="text-xs text-[#B8C6D8] mb-3 line-clamp-2">{{ tool.description }}</p>
                ...
                <!-- 操作按钮 -->
                <button class="w-full py-2 text-center bg-[#4A5F8B] text-[#F5F7FA] rounded-lg font-medium hover:bg-[#6B7C93] transition-colors text-sm">
          ...
          <div v-if="filteredAddedTools.length === 0" class="p-6 bg-[#2D3748] rounded-xl border border-[#4A5F8B] text-center">
            <div class="w-12 h-12 bg-[#1E2532] rounded-full flex items-center justify-center text-[#4A5F8B] mx-auto mb-3">
        ...
        <div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            >
              ...
              <div class="p-4">
                ...
                <!-- 操作按钮 -->
                <button class="w-full py-2 text-center bg-[#6B7C93] text-[#B8C6D8] rounded-lg font-medium hover:bg-[#4A5F8B] hover:text-[#F5F7FA] transition-colors text-sm">
          ...
          <div v-if="filteredRecommendedTools.length === 0" class="p-6 bg-[#2D3748] rounded-xl border border-[#4A5F8B] text-center">
            <div class="w-12 h-12 bg-[#1E2532] rounded-full flex items-center justify-center text-[#4A5F8B] mx-auto mb-3">
...
// 响应式状态
const activeTab = ref<'online' | 'presets'>('online')
...
// 标签分类（排除"全部"）
const tagsCategories = computed(() => categories.filter(cat => cat !== '全部'))
...
const filteredAddedTools = computed(() => {
  ...
  if (selectedTags.value.length > 0) {
    tools = tools.filter(tool => 
      selectedTags.value.some(tag => tool.category === tag)
    )
  }
