      ...
      <div class="bg-[#2D3748] rounded-xl p-6 shadow-sm border border-[#4A5F8B] mb-8">
        <div class="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4">
          ...
          <div class="flex space-x-4">
            ...
            >
              <template v-for="category in categories" :key="">
        ...
        <div class="mt-4">
          ...
          <div class="flex flex-wrap gap-2">
            <template v-for="category in categories.filter((cat) => cat !== '全部')" :key="">
              <button
                :key="category"
      ...
      <template v-if="activeTab === 'online'">
        ...
        <div class="mb-10">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <template v-for="tool in filteredAddedTools" :key="">
              <motion.div
                :key="tool.id"
                :whileHover="{
                  y: -5,
        ...
        <div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <template v-for="tool in filteredRecommendedTools" :key="">
              <motion.div
                :key="tool.id"
                :whileHover="{
                  y: -5,
...
<script setup lang="ts">
  ...
  const getFilteredAddedTools = () => {
    let tools = [...mockAddedTools]; // 按分类过滤
    ...
    if (selectedTags.length > 0) {
      tools = tools.filter((tool) => selectedTags.some((tag) => tool.category === tag));
    } // 排序
  ...
  const getFilteredRecommendedTools = () => {
    let tools = [...mockRecommendedTools]; // 按分类过滤
