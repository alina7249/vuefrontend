        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between">
          ...
          <div class="mb-6 md:mb-0">
            ...
            <div class="flex justify-between text-sm">
              <span
                >成长值: {{ mockGrowthData.currentLevel.progress }}/{{ mockGrowthData.currentLevel.maxProgress }}</span
              >
              <span
                >距离升级还需:
                {{ mockGrowthData.currentLevel.maxProgress - mockGrowthData.currentLevel.progress }}点</span
              >
            </div>
          </div>
      ...
      <template v-if="activeTab === 'growth'">
        <div class="space-y-8">
          ...
          <div class="bg-[#2D3748] rounded-xl p-6 shadow-sm border border-[#4A5F8B]">
            ...
            <div class="space-y-4">
              <template v-for="(item, index) in mockGrowthData.growthHistory" :key="index">
                <div
                  :key="index"
                  class="flex items-center justify-between p-4 bg-[#2D3748] rounded-lg border border-[#4A5F8B] hover:border-[#4A5F8B] hover:border-2 transition-all"
                >
        ...
        <div class="space-y-8">
          ...
          <div class="bg-[#2D3748] rounded-xl p-6 shadow-sm border border-[#4A5F8B]">
              ...
              <div class="flex items-center">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                  <input
                    ...
                    @change="() => (state.showClaimedRewards = !showClaimedRewards)"
                  />
                  <div
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <template v-for="reward in filteredRewards" :key="">
                <motion.div
                  :key="reward.id"
                  :whileHover="{
                    y: -5,
                  ...
                  :class="`bg-[#1E2532] rounded-xl overflow-hidden border transition-all ${
                    reward.available ? 'border-[#4A5F8B]' : 'border-[#4A5F8B]/50 opacity-80'
                  }`"
                >
                  <div class="relative">
                    ...
                    <template v-if="!reward.available">
                    ...
                    <div class="flex justify-between items-center">
                      ...
                      <div
                        :class="`text-sm font-medium ${
                          totalPoints >= reward.points && reward.available ? 'text-[#4A5F8B]' : 'text-[#ED8936]'
                        }`"
                      >
                        <template v-if="reward.available && totalPoints < reward.points"> </template>
                      </div>
                    </div>
                  </div>
                </motion.div>
            ...
            <template v-if="filteredRewards.length === 0">
      ...
      <template v-if="activeTab === 'tasks'">
        <div class="space-y-8">
          ...
          <div class="bg-[#2D3748] rounded-xl p-6 shadow-sm border border-[#4A5F8B]">
            ...
            <div class="space-y-4">
              <template v-for="task in mockGrowthData.ongoingTasks" :key="">
                <div :key="task.id" class="p-4 bg-[#1E2532] rounded-lg border border-[#4A5F8B]">
                  <div class="flex items-start justify-between mb-3">
                    <div class="flex items-start">
                      <div
                        class="w-8 h-8 rounded-full bg-[#1E2532] text-[#4A5F8B] border border-[#4A5F8B] flex items-center justify-center mr-3 flex-shrink-0"
                      >
          ...
          <div class="bg-[#2D3748] rounded-xl p-6 shadow-sm border border-[#4A5F8B]">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <template v-for="task in mockGrowthData.completedTasks" :key="">
                <div
                  :key="task.id"
                  class="p-4 bg-[#1E2532] rounded-lg border border-[#4A5F8B] flex items-center justify-between"
                >
                  <div class="flex items-center">
                    <div
                      class="w-10 h-10 rounded-full bg-[#4A5F8B]/20 flex items-center justify-center text-[#4A5F8B] mr-4"
...
<script setup lang="ts">
    ...
    availableRewards: [
      {
        ...
        points: 50,
        image:
          'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=photo%20preset%20pack%20thumbnail%20photography%20editing&sign=e8c6e47f437ba890910bd7f636a11f1b',
        available: true,
      },
      {
        id: 'r2',
        name: 'RAW素材下载券',
        description: '5个高质量RAW素材下载权限',
        points: 80,
        image:
          'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=raw%20photo%20download%20voucher%20thumbnail&sign=05dbb388d7010c5c0355b7ca1fcf8775',
        available: true,
      },
      {
        id: 'r3',
        name: '摄影课程折扣券',
        description: '线上摄影课程8折优惠',
        points: 100,
        image:
          'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=photography%20course%20discount%20voucher%20thumbnail&sign=9ab6fe1c09a5f10cf322494fa9d02fdb',
        available: true,
      },
      {
        id: 'r4',
        name: '高级会员体验卡',
        description: '7天高级会员免费体验',
        points: 150,
        image:
          'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=premium%20membership%20trial%20card%20thumbnail%20gold%20glow&sign=1c989a761cbbc5239aefecefd0fb57bf',
        available: true,
      },
      {
        id: 'r5',
        name: '专业器材租赁券',
        description: '专业相机镜头租赁一天免费',
        points: 200,
        image:
          'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=camera%20equipment%20rental%20voucher%20thumbnail&sign=526fa89bffb356b8ee6de91d54131e4f',
        available: false,
      },
      {
        id: 'r6',
        name: '线下活动免费券',
        description: '免费参加一次线下摄影活动',
        points: 250,
        image:
          'https://space.coze.cn/api/coze_space/gen_image?image_size=square&prompt=photography%20event%20free%20ticket%20thumbnail&sign=603c66e6e40f8e7fb5a2c9abab507bff',
        available: false,
      },
    ],
    completedTasks: [
  ...
  // 计算已获得和可获得的成长值
  const totalPoints = mockGrowthData.growthHistory.reduce((sum, item) => sum + item.points, 0);
  const totalAvailableRewardsPoints = mockGrowthData.availableRewards
    .filter((reward) => reward.available)
    .reduce((sum, reward) => sum + reward.points, 0); // 过滤奖励
  // 过滤奖励
