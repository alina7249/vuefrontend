  ...
  <div v-else class="container mx-auto px-4 py-8 bg-[#1E2532] star-texture min-h-screen">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div class="lg:col-span-3 space-y-8">
          ...
          <div class="bg-[#2D3748] rounded-xl p-6 shadow-sm border border-[#4A5F8B]">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
              <!--  作品管理  -->
              <div class="bg-[#1E2532] rounded-lg p-5 border border-[#4A5F8B] hover:border-[#6B7C93] transition-colors">
                <div class="flex items-center justify-between mb-4">
                  <h3 class="text-base font-medium text-[#F5F7FA] flex items-center">
                    <i class="fa-solid fa-camera text-[#4A5F8B] mr-2"></i>
                    作品管理
                  </h3>
                  <RouterLink to="/profile-center/works" class="text-xs text-[#4A5F8B] hover:text-[#6B7C93]">
                    查看全部
                  </RouterLink>
                </div>
                ...
                <div class="grid grid-cols-3 gap-2 mb-4">
                  <template v-for="work in recentWorks" :key="work.id">
                    <div
                      v-motion:hover="{
                        scale: 1.05,
                      }"
              ...
              <div class="bg-[#1E2532] rounded-lg p-5 border border-[#4A5F8B] hover:border-[#6B7C93] transition-colors">
                <div class="flex items-center justify-between mb-4">
                  ...
                  <RouterLink to="/profile-center/materials" class="text-xs text-[#4A5F8B] hover:text-[#6B7C93]">
                    查看全部
                  </RouterLink>
                </div>
                ...
                <div class="space-y-3 mb-4">
                  <template v-for="equipment in recentEquipment" :key="equipment.id">
              ...
              <div class="bg-[#1E2532] rounded-lg p-5 border border-[#4A5F8B] hover:border-[#6B7C93] transition-colors">
                <div class="flex items-center justify-between mb-4">
                  ...
                  <RouterLink to="/online-courses" class="text-xs text-[#4A5F8B] hover:text-[#6B7C93]">
                    查看全部
                  </RouterLink>
                </div>
                ...
                <div class="space-y-4 mb-4">
                  <template v-for="course in recentCourses" :key="course.id">
                    <div class="space-y-1">
          ...
          <div class="bg-gradient-to-r from-[#4A5F8B] to-[#6B7C93] rounded-xl p-6 shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div class="bg-[#1E2532]/70 rounded-lg p-5">
                ...
                <div class="space-y-3">
                  <template v-for="benefit in memberBenefits" :key="benefit.id">
                    <div class="flex items-center justify-between">
                      <div class="flex items-center">
                        <div
              ...
              <div class="bg-[#1E2532]/70 rounded-lg p-5">
                ...
                <div class="space-y-4">
                  <template v-for="item in benefitUsage" :key="item.id">
                    <div class="space-y-1">
                      <div class="flex justify-between items-center">
              ...
              <div class="bg-[#1E2532]/70 rounded-lg p-5">
                ...
                <div class="space-y-4">
                  <template v-for="event in memberEvents" :key="event.id">
                    <div class="p-3 bg-[#1E2532] rounded-lg">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div class="bg-[#1E2532] rounded-lg p-5 border border-[#4A5F8B]">
                <div class="flex items-center justify-between mb-4">
                  ...
                  <RouterLink to="/profile-center/benefits" class="text-xs text-[#4A5F8B] hover:text-[#6B7C93]">
                    查看全部
                  </RouterLink>
                </div>
                <div class="space-y-4">
                  <template v-for="task in growthTasks" :key="task.id">
                    <div>
              ...
              <template v-for="item in sidebarItems" :key="item.id">
                <RouterLink
                  ...
                  :class="`w-full text-left px-4 py-3 rounded-lg transition-colors flex items-center ${
                    activeSidebarItem === item.id ? 'bg-[#4A5F8B] text-[#F5F7FA]' : 'text-[#B8C6D8] hover:bg-[#1E2532] hover:text-[#F5F7FA]'
                  }`"
                >
    ...
    <div class="lg:hidden fixed bottom-0 left-0 right-0 bg-[#2D3748] border-t border-[#4A5F8B] z-40">
      <div class="grid grid-cols-4 py-2">
        <template v-for="item in sidebarItems.slice(0, 4)" :key="item.id">
          <RouterLink
            :to="
              item.id === '作品集'
                ? '/profile-center/works'
...
interface Activity {
  id: number
  text: string
  type: 'post' | 'follower' | 'task' | 'featured' | 'contest'
}
...
const sidebarItems: SidebarItem[] = [
  { id: '作品集', icon: 'fa-camera', text: '我的作品集' },
  { id: '拍摄地点', icon: 'fa-map-marker-alt', text: '我的拍摄地点' },
  { id: '器材库', icon: 'fa-video', text: '我的器材库' },
  { id: '素材库', icon: 'fa-microchip', text: '我的素材库' },
  { id: '我的活动', icon: 'fa-flag', text: '我的活动' },
  { id: '后期工具', icon: 'fa-palette', text: '后期工具' },
  { id: '会员中心', icon: 'fa-crown', text: '会员中心' },
  { id: '我的订单', icon: 'fa-shopping-cart', text: '我的订单' },
  { id: '成长福利', icon: 'fa-award', text: '成长福利' },
  { id: '设置', icon: 'fa-cog', text: '设置' },
]

// 会员权益
const memberBenefits: MemberBenefit[] = [
  { id: 1, name: '免费RAW素材', count: 10 },
  { id: 2, name: '赛事优先报名', count: null },
  { id: 3, name: '专属后期预设包', count: null },
  { id: 4, name: '作品优先推荐', count: null },
]
</script>
