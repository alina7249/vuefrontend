      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <div class="space-y-6">
                ...
                <div class="p-5 md:w-2/3">
                  ...
                  <div class="flex items-center justify-between mb-2">
                    <div class="flex space-x-2">
                      ...
                      <span class="text-xs px-2 py-1 bg-[#E6EBF2] text-[#4A5F8B] rounded-full">{{ request.category }}</span>
                    </div>
                  ...
                  <div class="flex flex-wrap gap-2 mb-4">
                    <button
                      v-for="(tag, index) in request.tags.slice(0, 5)"
                      :key="index"
                      @click="toggleTag(tag)"
                      :class="`px-2 py-1 rounded-full text-xs ${
                        selectedTags.includes(tag)
                          ? 'bg-[#4A5F8B] text-[#F5F7FA]'
                          : 'bg-[#E6EBF2] text-[#4A5059] border border-[#B8C6D8]/30 hover:bg-[#4A5F8B] hover:text-[#F5F7FA]'
                      } transition-colors`"
                    >
                      #{{ tag }}
                    </button>
            ...
            <div v-if="filteredRequests.length === 0" class="p-8 bg-[#F5F7FA] rounded-xl border border-[#B8C6D8] text-center">
          ...
          <!-- 分页 -->
          <div v-if="filteredRequests.length > 0" class="flex justify-center">
        ...
        <div class="lg:col-span-1 space-y-6">
          ...
          <div class="bg-[#F5F7FA] rounded-xl p-6 shadow-sm border border-[#B8C6D8]">
            ...
            <div class="mb-6">
              ...
              <div class="space-y-2">
                <div v-for="range in budgetRanges" :key="range" class="flex items-center">
                  <input
                    type="radio"
                    :id="`budget-${range}`"
                    name="budget-range"
                    :checked="selectedBudgetRange === range"
                    @change="selectedBudgetRange = range"
            ...
            <div class="mb-6">
              ...
              <div class="space-y-2">
                <div v-for="location in ['全部', '北京', '上海', '广州', '深圳', '成都', '杭州', '其他']" :key="location" class="flex items-center">
                  <input
                    type="radio"
            ...
            <div class="mb-6">
              ...
              <div class="space-y-2">
                <div v-for="time in ['全部', '今天', '本周', '本月', '三个月内']" :key="time" class="flex items-center">
                  <input
                    type="radio"
                    :id="`time-${time}`"
                    name="time"
...
const selectedBudgetRange = ref('全部')
const searchTerm = ref('')
const sortBy = ref('newest') // newest, popular, budget-asc, budget-desc
...
const filteredRequests = computed(() => {
  ...
  if (selectedType.value !== '全部') {
    requests = requests.filter(request => request.type === selectedType.value)
  }
  
  // 按分类过滤
  if (selectedCategory.value !== '全部') {
    requests = requests.filter(request => request.category === selectedCategory.value)
  }
  
  // 按预算范围过滤
  if (selectedBudgetRange.value !== '全部') {
  ...
  if (selectedTags.value.length > 0) {
    requests = requests.filter(request => 
      selectedTags.value.some(tag => request.tags.includes(tag))
    )
  }
...
.star-texture {
  background-image: radial-gradient(circle at 1px 1px, rgba(184, 198, 216, 0.15) 1px, transparent 0);
